"""
Run Tasks 8-10 of the conformal calibration plan.
Fit production models, serialize them, create visualizations, and summary tables.
"""
...
# Load the notebook to extract the FINAL_COVERAGE dict and other context
nb_path = ROOT / "notebooks" / "04_conformal.ipynb"
...
# Read the notebook cells to extract variable definitions
# We need: model_df, FEATURES_VANILLA, H, XGB_PARAMS
# For now, we'll reconstruct these from the data
...
print(f"Panel loaded: {panel.shape[0]} rows")
...
# Now we need to extract FINAL_COVERAGE from the notebook outputs
# Look for cell 9 output which contains the final coverage data
print("\n" + "=" * 80)
print("EXTRACTING COVERAGE STATISTICS FROM NOTEBOOK")
print("=" * 80)

# For now, we'll create a minimal FINAL_COVERAGE structure
# In a real scenario, this would come from cell 12 execution
FINAL_COVERAGE = {
...
print("\nNote: To complete visualizations (Tasks 9-10), the notebook cells 1-12")
print("must be executed first to populate FINAL_COVERAGE with actual coverage data.")
print("\nPlease run the notebook manually or integrate execution environment.")
