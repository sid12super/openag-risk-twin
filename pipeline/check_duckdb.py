...
DATE_COL = {"prices": "Date", "wasde": "ReportDate", "cftc": "report_date",
            "drought": "mapDate", "macro": "date"}

def main():
    con = duckdb.connect("data/openag.duckdb", read_only=True)
    for t, dc in DATE_COL.items():
        n = con.sql(f"SELECT COUNT(*) FROM {t}").fetchone()[0]
        lo, hi = con.sql("SELECT MIN(strptime(ReportDate,'%B %Y')), MAX(strptime(ReportDate,'%B %Y')) FROM wasde").fetchone()
